...
static inline void fill_mat_values_direct_Natural(PetscScalar *State, PetscInt i, PetscInt center, PetscInt j, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt maxcols,PetscScalar *B, PetscScalar *C, PetscScalar dt, PetscInt *cols, PetscScalar *vals)
{ 
...
static inline void fill_mat_values_direct_Neumann(PetscScalar *State, PetscInt i, PetscInt center, PetscInt j, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt maxcols,PetscScalar *B, PetscScalar *C, PetscScalar dt, PetscInt *cols, PetscScalar *vals)
{ 
  PetscScalar firstCoefE = B[i],secondCoefE = C[i];
...
static inline void fill_mat_values_CrossDiff_Neumann(PetscScalar *StateX, PetscScalar *StateY, PetscInt i, PetscInt center, PetscInt jX, PetscInt jY, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt maxcols, PetscScalar *C_XY, PetscScalar dt, PetscInt *cols, PetscScalar *vals)
{ 
...
/************************************************/
  ...
  } else if (  PetscAbs(StateX[i]-lowerLims[jX]) > dVecX/2.0  && PetscAbs(StateX[i]-upperLims[jX]) > dVecX/2.0 && PetscAbs(StateY[i]-upperLims[jY]) < dVecY /2.0 )  {
  // case 2: top
/************************************************/
  // case 2: edge line
/************************************************/
  ...
  } else if (  PetscAbs(StateX[i]-lowerLims[jX]) > dVecX/2.0  && PetscAbs(StateX[i]-upperLims[jX]) > dVecX/2.0 && PetscAbs(StateY[i]-upperLims[jY]) > dVecY /2.0 &&PetscAbs(StateY[i]-lowerLims[jY]) > dVecY /2.0 )  {
/************************************************/
  // case 3: interior
/************************************************/
    vals[center+1+4*jX] += -dt*(  -crossCoefE/(4*dVecX*dVecY) );
...
static inline void fill_mat_values_CrossDiff_Natural(PetscScalar *StateX, PetscScalar *StateY, PetscInt i, PetscInt center, PetscInt jX, PetscInt jY, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt maxcols, PetscScalar *C_XY, PetscScalar dt, PetscInt *cols, PetscScalar *vals)
{ 
  PetscScalar crossCoefE = C_XY[i];
  PetscScalar dVecX = dVec[jX];
  PetscScalar dVecY = dVec[jY];
...
/************************************************/
  ...
  // case 1: corner point  // top right 
    ...
    vals[center] += -dt*(  crossCoefE/(4*dVecX*dVecY) ); 
    vals[center-2-2*4*jX] += -dt*(  -crossCoefE/(4*dVecX*dVecY) );
    vals[center-4-2*4*jX] += -dt*(  -crossCoefE/(4*dVecX*dVecY) );

    vals[center+4+2*4*jX] += -dt*(  crossCoefE/(4*dVecX*dVecY) );
    
    cols[center-2-2*4*jX] = i - 2*incVec[jX];
    cols[center-4-2*4*jX] = i - 2*incVec[jY];
    cols[center+4+2*4*jX] = i - 2*incVec[jX]-2*incVec[jY];
  ...
  // case 1: corner point  // bottom right 
    ...
    vals[center] += -dt*(  -crossCoefE/(4*dVecX*dVecY) ); 
    vals[center-1-2*4*jX] += -dt*(  crossCoefE/(4*dVecX*dVecY) );
    vals[center-2-2*4*jX] += -dt*(  crossCoefE/(4*dVecX*dVecY) );

    vals[center+1+2*4*jX] += -dt*(  -crossCoefE/(4*dVecX*dVecY) );
    
    cols[center-1-2*4*jX] = i + 2*incVec[jY];
    cols[center-2-2*4*jX] = i - 2*incVec[jX];
    cols[center+1+2*4*jX] = i - 2*incVec[jX]+2*incVec[jY];
  ...
  // case 1: corner point  // top left 
    ...
    vals[center] += -dt*(  -crossCoefE/(4*dVecX*dVecY) ); 
    vals[center-3-2*4*jX] += -dt*(  crossCoefE/(4*dVecX*dVecY) );
    vals[center-4-2*4*jX] += -dt*(  crossCoefE/(4*dVecX*dVecY) );

    vals[center+3+2*4*jX] += -dt*(  -crossCoefE/(4*dVecX*dVecY) );
    
    cols[center-3-2*4*jX] = i + 2*incVec[jX];
    cols[center-4-2*4*jX] = i - 2*incVec[jY];
    cols[center+3+2*4*jX] = i + 2*incVec[jX]-2*incVec[jY];
  ...
  // case 1: corner point  // bottom left 
    ...
    cols[center-1-2*4*jX] = i + 2*incVec[jY];
    cols[center-3-2*4*jX] = i + 2*incVec[jX];
    cols[center+2+2*4*jX] = i + 2*incVec[jY] + 2*incVec[jX] ;
...
/************************************************/
    ...
    cols[center-2-2*4*jX] = i - incVec[jX];
    cols[center-3-2*4*jX] = i + incVec[jX];
    cols[center+3+2*4*jX] = i + incVec[jX]-2*incVec[jY];
    cols[center+4+2*4*jX] = i - incVec[jX]-2*incVec[jY];
  ...
  // case 2: bottom
    ...
    cols[center-2-2*4*jX] = i - incVec[jX];
    cols[center-3-2*4*jX] = i + incVec[jX];
    cols[center+1+2*4*jX] = i - incVec[jX]+2*incVec[jY];
    cols[center+2+2*4*jX] = i + incVec[jX]+2*incVec[jY];
  ...
  // case 2: left
    ...
    cols[center-1-2*4*jX] = i + incVec[jY];
    cols[center-4-2*4*jX] = i - incVec[jY];
    cols[center+2+2*4*jX] = i + 2*incVec[jX]+incVec[jY];
    cols[center+3+2*4*jX] = i + 2*incVec[jX]-incVec[jY];
  ...
  // case 2: right
    ...
    cols[center-1-2*4*jX] = i + incVec[jY];
    cols[center-4-2*4*jX] = i - incVec[jY];
    cols[center+1+2*4*jX] = i - 2*incVec[jX]+incVec[jY];
    cols[center+4+2*4*jX] = i - 2*incVec[jX]-incVec[jY];
...
/************************************************/
    ...
    cols[center+1+2*4*jX] = i - incVec[jX] + incVec[jY] ;
    cols[center+2+2*4*jX] = i + incVec[jX] + incVec[jY];
    cols[center+3+2*4*jX] = i + incVec[jX] - incVec[jY];
    cols[center+4+2*4*jX] = i - incVec[jX] - incVec[jY];
...
PetscErrorCode FormLinearSystem_DirectCrossDiff_Neumann(PetscScalar *R, PetscScalar *F, PetscScalar *K, PetscScalar *A, PetscScalar *B_r, PetscScalar *B_f, PetscScalar *B_k, PetscScalar *C_rr, PetscScalar *C_ff, PetscScalar *C_kk, PetscScalar *C_rf, PetscScalar *C_fk, PetscScalar *C_kr, PetscScalar dt, PetscScalar *lowerLims, PetscScalar *upperLims, PetscScalar *dVec, PetscInt *incVec, PetscInt n, Mat petsc_mat)
{
  ...
  for (i = 0; i < n; ++i) {
    ...
    fill_mat_values_CrossDiff_Neumann(R,F, iXDiff,centerXDiff,0, 1 ,lowerLims,upperLims,dVec,incVec,n,C_rf,dt,colsXDiff,valsXDiff);
    fill_mat_values_CrossDiff_Neumann(F,K, iXDiff,centerXDiff,1, 2 ,lowerLims,upperLims,dVec,incVec,n,C_fk,dt,colsXDiff,valsXDiff);
    fill_mat_values_CrossDiff_Neumann(K,R, iXDiff,centerXDiff,2, 0 ,lowerLims,upperLims,dVec,incVec,n,C_kr,dt,colsXDiff,valsXDiff);
